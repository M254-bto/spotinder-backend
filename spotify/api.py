...
def pre_processing(data):
    drop_cols = [
        x
        for x in ["year", "name", "explicit", "id", "release_date", "artists"]
    ...
    scaler = pd.DataFrame(
        MinMaxScaler().fit_transform(data[scale_cols]), columns=scale_cols
    )
    standard = pd.DataFrame(
        StandardScaler().fit_transform(data[z_cols]), columns=z_cols
    )
...
model = pickle.load(
    open(
        "/content/drive/MyDrive/Colab Notebooks/Spotify classifier/song_cluster.pkl",
        "rb",
    )
)
